...
export function useConfig() {
  ...
  const saveConfig = useCallback(async (newConfig: GlobalConfig): Promise<boolean> => {
    try {
      setError(null)
  ...
  const updateSettings = useCallback(async (
    settings: Partial<GlobalConfig['settings']>
  ): Promise<boolean> => {
...
export function useAppConfig() {
  ...
  const addApp = useCallback(async (appConfig: AppConfig): Promise<GlobalConfig | null> => {
    try {
      setIsOperating(true)
      setError(null)
  ...
  const updateApp = useCallback(async (appConfig: AppConfig): Promise<GlobalConfig | null> => {
    try {
      setIsOperating(true)
      setError(null)
  ...
  const removeApp = useCallback(async (appId: string): Promise<GlobalConfig | null> => {
    try {
      setIsOperating(true)
      setError(null)
  ...
  const getAppById = useCallback((config: GlobalConfig | null, appId: string): AppConfig | null => {
    if (!config) return null
    return config.apps.find(app => app.id === appId) || null
  }, [])

  // Check if app name already exists (for validation)
  const isAppNameTaken = useCallback((config: GlobalConfig | null, name: string, excludeId?: string): boolean => {
    if (!config) return false
    return config.apps.some(app => app.name === name && app.id !== excludeId)
  }, [])

  // Duplicate an app configuration with new ID and modified name
  const duplicateApp = useCallback(async (
...
export function useConfigOperations() {
  ...
  const restoreFromBackup = useCallback(async (backupPath: string): Promise<GlobalConfig | null> => {
    try {
      setIsOperating(true)
      setError(null)
      
      const result = await invoke<GlobalConfig>('restore_config', { backupPath })
      return result
    } catch (err) {
...
export function useConfigManager() {
  ...
  const saveAppAndRefresh = useCallback(async (
    operation: () => Promise<GlobalConfig | null>
  ): Promise<boolean> => {
  ...
  return {
    ...
    addApp: useCallback(async (appConfig: AppConfig) => 
      saveAppAndRefresh(() => appConfigHook.addApp(appConfig)), 
      [saveAppAndRefresh, appConfigHook]
    ),
    updateApp: useCallback(async (appConfig: AppConfig) => 
      saveAppAndRefresh(() => appConfigHook.updateApp(appConfig)), 
      [saveAppAndRefresh, appConfigHook]
    ),
    removeApp: useCallback(async (appId: string) => 
      saveAppAndRefresh(() => appConfigHook.removeApp(appId)), 
      [saveAppAndRefresh, appConfigHook]
    ),
    duplicateApp: useCallback(async (appId: string) => 
      saveAppAndRefresh(() => appConfigHook.duplicateApp(configHook.config, appId)), 
      [saveAppAndRefresh, appConfigHook, configHook.config]
    ),
    
    // App utilities
    getAppById: useCallback((appId: string) => 
      appConfigHook.getAppById(configHook.config, appId), 
      [appConfigHook, configHook.config]
    ),
    isAppNameTaken: useCallback((name: string, excludeId?: string) => 
